...
public class PrologTextLoader
{
	...
	public PrologTextLoader(PrologTextLoaderState prologTextLoaderState, Term root)
	{
		...
		try
		{
			currentReader = new TermReader(new InputStreamReader(prologTextLoaderState.getInputStream(root), StandardCharsets.UTF_8),
	...
	protected void processFile()
	{
		...
		while (currentReader != null)
		{
			...
			switch (term) {
				case null -> processEof();
				case AtomTerm at -> processClause(term);
				case CompoundTerm ct when ct.tag != TermConstants.directiveTag -> processClause(term);
				case CompoundTerm ct -> {
					if (!(ct.args[0] instanceof CompoundTerm dirTerm)) {
						logError("invalid directive term");
					} else {
						processDirective(dirTerm);
					}
				}
				default -> logError("term is not a clause or directive");
			}
		}
		prologTextLoaderState.afterProcessFile(this);
	}

	protected void processDirective(final CompoundTerm dir)
	{
		final var tag = dir.tag;
		if (tag == includeTag) {
			processIncludeDirective(dir.args[0]);
		} else if (tag == multifileTag) {
			processMultifileDirective(dir.args[0]);
		} else if (tag == dynamicTag) {
			processDynamicDirective(dir.args[0]);
		} else if (tag == discontiguousTag) {
			processDiscontiguousDirective(dir.args[0]);
		} else if (tag == opTag) {
			processOpDirective(dir.args[0], dir.args[1], dir.args[2]);
		} else if (tag == char_conversionTag) {
			processCharConversionDirective(dir.args[0], dir.args[1]);
		} else if (tag == initializationTag) {
			processInitializationDirective(dir.args[0]);
		} else if (tag == ensure_loadedTag) {
			processEnsureLoadedDirective(dir.args[0]);
		} else if (tag == set_prolog_flagTag) {
			processSetPrologFlagDirective(dir.args[0], dir.args[1]);
		} else if (tag == externalTag) {
			processExternalDirective(dir.args[0], dir.args[1]);
		} else if (tag == controlTag) {
			processControlDirective(dir.args[0], dir.args[1]);
		} else if (tag == build_inTag) {
			processBuildInDirective(dir.args[0], dir.args[1]);
		} else {
			// Treat it as a goal to run at runtime - not in ISO but common (NONISO)
			processInitializationDirective(dir);
		}
	}
	...
	{
		if (!(className instanceof AtomTerm classNameAtom))
		{
			logError("class name should be atom term");
			return;
		}
		if (!(pi instanceof CompoundTerm piTerm))
		{
			logError("predicate indicator should be a compound term");
			return;
		}
		prologTextLoaderState.defineExternal(this, piTerm, classNameAtom.value,
	...
	protected void processCharConversionDirective(final Term from, final Term to)
	{
		final var cfrom = switch (from) {
			case AtomTerm at when at.value.length() == 1 -> at.value.charAt(0);
			case AtomTerm at -> { logError("Should be a single length atom"); yield null; }
			case VariableTerm vt -> { logError("instantiation error"); yield null; }
			default -> { logError("representation error"); yield null; }
		};
		if (cfrom == null) return;

		final var cto = switch (to) {
			case AtomTerm at when at.value.length() == 1 -> at.value.charAt(0);
			case AtomTerm at -> { logError("Should be a single length atom"); yield null; }
			case VariableTerm vt -> { logError("instantiation error"); yield null; }
			default -> { logError("representation error"); yield null; }
	...
	{
		if (!(specifier instanceof AtomTerm specifierAtom))
		{
			logError("the specifier should be an atom term");
			return;
		}
		if (!(priority instanceof IntegerTerm priorityInt))
		{
			logError("the priority should be an integer term");
			return;
		}

		if (!(operatorAtom instanceof AtomTerm operatorAtomTerm))
		{
		...
		operatorSet.add(priorityInt.value, spec, operatorAtomTerm.value);
	...
	{
		// Check if this is a DCG rule and transform it
		if (DCGTransformer.isDCGRule(argument))
		{
			Term transformedClause = DCGTransformer.transform(argument);
			prologTextLoaderState.addClause(this, transformedClause);
		}
		else
		{
	...
	protected void processIncludeDirective(Term argument)
	{
		try
		{
			...
			final var reader = new TermReader(new InputStreamReader(prologTextLoaderState.getInputStream(argument), StandardCharsets.UTF_8),
					prologTextLoaderState.getEnvironment());
