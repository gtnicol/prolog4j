...
final public class CompoundTermTag implements java.io.Serializable
{
	...
	/** atom to tag map */
	private static final ConcurrentHashMap<AtomTerm, List<CompoundTermTag>> atom2tag = new ConcurrentHashMap<>();
	...
	{
		if (!(predicateIndicator.args[0] instanceof AtomTerm at) || !(predicateIndicator.args[1] instanceof IntegerTerm it))
		{
			throw new IllegalArgumentException();
		}
		return get(at, it.value);
	}
	 ...
	 * @param arity
	 *          arity of tag
	 * @return a tag that have specified arity and functor
	 */
	public static CompoundTermTag get(final AtomTerm functor, final int arity)
	{
		final var tags = atom2tag.computeIfAbsent(functor, k -> new CopyOnWriteArrayList<>());
		...
		// Need to create a new tag - use synchronization to prevent duplicates
		synchronized (tags)
		{
			...
			final var fresh = new CompoundTermTag(functor, arity);
			tags.add(fresh);
			return fresh;
		}
	}
