 ...
 */
public abstract class AbstractDateTimePredicate implements ExecuteOnlyCode
{
	...
	private static final AtomTerm DASH = AtomTerm.get("-");
	private static final Set<String> AVAILABLE_TIME_ZONES = Collections.unmodifiableSet(
			new HashSet<>(Arrays.asList(TimeZone.getAvailableIDs())));

	public static Date getDate(Term term) throws PrologException
	{
		...
		Term dstFlag = cterm.args[8];
		// get the timezone
		TimeZone zone = TimeZone.getDefault();
		Term timezoneTerm = cterm.args[7];
		if (timezoneTerm != DASH)
		{
			AtomTerm zoneAtom = (AtomTerm) timezoneTerm;
			String zoneId = zoneAtom.value;
			if (!AVAILABLE_TIME_ZONES.contains(zoneId))
			{
				PrologException.domainError(TermConstants.timeZoneAtom, timezoneTerm);
			}
		...
		else
		{
			...
			for (String zoneId : zones)
			{
				...
				else if (dstFlag == TermConstants.trueAtom)
				{
				...
				else if (dstFlag == TermConstants.falseAtom)
				{
					if (!z.useDaylightTime())
					{
						zone = z;
		...
		if (dstFlag != DASH && dstFlag != TermConstants.trueAtom && dstFlag != TermConstants.falseAtom)
		{
			PrologException.domainError(TermConstants.daylightSavingAtom, dstFlag);
		}
		Calendar cal = Calendar.getInstance(zone);
		cal.clear();
		int sec = 0;
		int msec = 0;
		if (cterm.args[5] instanceof IntegerTerm intTerm)
		{
			sec = intTerm.value;
		}
		else
		{
			FloatTerm floatTerm = (FloatTerm) cterm.args[5];
			sec = (int) Math.floor(floatTerm.value);
			msec = (int) Math.round((floatTerm.value % 1.0) * 1000);
		}
	...
	public AbstractDateTimePredicate()
	{}
}
