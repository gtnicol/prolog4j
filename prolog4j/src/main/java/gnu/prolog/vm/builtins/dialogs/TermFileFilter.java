...
public class TermFileFilter extends FileFilter
{
	...
	public TermFileFilter(Term term) throws PrologException
	{
		...
		else if (term instanceof CompoundTerm ct && ct.tag.arity == 1)
		{
			pattern = ct.tag.functor.value;
			term = ct.args[0].dereference();
			if (term instanceof AtomTerm at2)
			{
				description = at2.value;
			}
			else
			{
		...
		{
			PrologException.domainError(Predicates.DIALOG_OPTION_ATOM, term);
		}
		...
		{
			PrologException.domainError(Predicates.FILEMASK_TAG.functor, term);
		}
