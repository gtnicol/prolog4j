...
public class InterpretedCodeCompiler
{
	...
	{
		switch (term) {
			case VariableTerm vt -> iPushEnvironment(getEnvironmentIndex(vt));
			case AtomicTerm at -> iPushConstant(at);
			...
			default -> PrologException.systemError();
		}
	}
	...
	void compileHead(Term headTerm) throws PrologException
	{
		switch (headTerm) {
			case AtomTerm at -> {
				// do nothing
				if (codeTag == null) {
					codeTag = CompoundTermTag.get(at, 0);
				}
			}
			case CompoundTerm ct -> {
			...
			default -> PrologException.typeError(TermConstants.callableAtom, headTerm);
		}
	}
	...
	{
		switch (body) {
			case VariableTerm vt -> {  // all variable are converted to call(Var)
			...
			case AtomTerm at -> {
				if (body == TermConstants.cutAtom) { // cut
					...
					iCall(CompoundTermTag.get(at, 0));
				}
			}
			case CompoundTerm ct -> {
				...
				if (tag == TermConstants.conjunctionTag) {
					...
					if (ct.args[0] instanceof CompoundTerm ct2  // if then else
							&& ct2.tag == TermConstants.ifTag) {
			...
			default -> PrologException.typeError(TermConstants.callableAtom, body);
		}
	}
	...
	{
		int rc = 0;
		if (body instanceof CompoundTerm ct) {
			CompoundTermTag tag = ct.tag;
			if (tag == TermConstants.conjunctionTag) {
	...
	int getAllVariables(Term term, int currentEnvPositon)
	{
		switch (term) {
			...
			default -> {}  // do nothing for atomic terms
		}
	...
	public static Term rdereferenced(Term term)
	{
		...
		if (term instanceof CompoundTerm ct1) {
			int n = ct1.tag.arity;
			Term args1[] = ct1.args;
	...
	{
		if (clause instanceof CompoundTerm ct && ct.tag == TermConstants.clauseTag) {
	...
	PrologCode compilePredicate() throws PrologException
	{
		...
		else
		{
			...
			{
				List<IJump> jumps = new ArrayList<>();
	...
	int popCutPosition()
	{
		if (cutPositionStack.isEmpty())
		{
			throw new IllegalStateException("Cut position stack is empty");
		}
