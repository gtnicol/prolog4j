...
{
	private static final Logger logger = LoggerFactory.getLogger(InterpretedByteCode.class);
			...
			Map<AtomicTerm, Integer> constant2idx)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			bytes += switch (isrc[i]) {
				...
				case ICreateCompoundTerm icct -> {
					createCompoundTermTags.add(icct.tag);
					yield 3;
				}
				case ICreateVariable icv -> 1;
				case ICut icut -> 3;
				case IDup idup -> 1;
				case IFail ifail -> 1;
				case IJump ijump -> 3;
				case IPop ipop -> 1;
				case IPushArgument ipa -> 3;
				case IPushConstant ipc -> {
					constantSet.add(ipc.term);
					yield 3;
				}
				case IPushEnvironment ipe -> 3;
				case IRetryMeElse irme -> 3;
				case IReturn iret -> 1;
				case ISaveCut isc -> 3;
				case IStoreEnvironment ise -> 3;
				case IThrow ithrow -> 1;
				case ITrue itrue -> 1;
				case ITrustMe itm -> 1;
				case ITryMeElse itme -> 3;
				case IUnify iunify -> 1;
				default -> throw new IllegalStateException("Unknown instruction type: " + isrc[i].getClass());
			};
		}
		...
		for (i = 0; i < n; i++)
		{
			switch (isrc[i]) {
				case IAllocate ia -> {
					...
					instructions[bytes++] = (byte) (ia.environmentSize >> 8 & 255);
					instructions[bytes++] = (byte) (ia.environmentSize & 255);
					instructions[bytes++] = (byte) (ia.reserved >> 8 & 255);
					instructions[bytes++] = (byte) (ia.reserved & 255);
				}
				case ICall ic -> {
					int idx = (tag2idx.get(ic.tag)).intValue();
					instructions[bytes++] = (byte) ICALL;
					instructions[bytes++] = (byte) (idx >> 8 & 255);
					instructions[bytes++] = (byte) (idx & 255);
				}
				case ICreateCompoundTerm icct -> {
					int idx = (tag2idx.get(icct.tag)).intValue();
					instructions[bytes++] = (byte) ICREATE_COMPOUND;
					instructions[bytes++] = (byte) (idx >> 8 & 255);
					instructions[bytes++] = (byte) (idx & 255);
				}
				case ICreateVariable icv -> {
					instructions[bytes++] = (byte) ICREATE_VARIABLE;
				}
				case ICut icut -> {
					instructions[bytes++] = (byte) ICUT;
					instructions[bytes++] = (byte) (icut.environmentIndex >> 8 & 255);
					instructions[bytes++] = (byte) (icut.environmentIndex & 255);
				}
				case IDup idup -> {
				...
				case IJump ijump -> {
					...
					instructions[bytes++] = (byte) (ipos[ijump.jumpPosition] >> 8 & 255);
					instructions[bytes++] = (byte) (ipos[ijump.jumpPosition] & 255);
				}
				case IPop ipop -> {
					instructions[bytes++] = (byte) IPOP;
				}
				case IPushArgument ipa -> {
					instructions[bytes++] = (byte) IPUSH_ARGUMENT;
					instructions[bytes++] = (byte) (ipa.argumentPosition >> 8 & 255);
					instructions[bytes++] = (byte) (ipa.argumentPosition & 255);
				}
				case IPushConstant ipc -> {
					int idx = (constant2idx.get(ipc.term)).intValue();
					instructions[bytes++] = (byte) IPUSH_CONSTANT;
					instructions[bytes++] = (byte) (idx >> 8 & 255);
					instructions[bytes++] = (byte) (idx & 255);
				}
				case IPushEnvironment ipe -> {
					instructions[bytes++] = (byte) IPUSH_ENVIRONMENT;
					instructions[bytes++] = (byte) (ipe.environmentPosition >> 8 & 255);
					instructions[bytes++] = (byte) (ipe.environmentPosition & 255);
				}
				case IRetryMeElse irme -> {
					instructions[bytes++] = (byte) IRETRY_ME_ELSE;
					instructions[bytes++] = (byte) (ipos[irme.retryPosition] >> 8 & 255);
					instructions[bytes++] = (byte) (ipos[irme.retryPosition] & 255);
				}
				case IReturn iret -> {
				...
				case ISaveCut isc -> {
					...
					instructions[bytes++] = (byte) (isc.environmentIndex >> 8 & 255);
					instructions[bytes++] = (byte) (isc.environmentIndex & 255);
				}
				case IStoreEnvironment ise -> {
					instructions[bytes++] = (byte) ISTORE_ENVIRONMENT;
					instructions[bytes++] = (byte) (ise.environmentIndex >> 8 & 255);
					instructions[bytes++] = (byte) (ise.environmentIndex & 255);
				}
				case IThrow ithrow -> {
				...
				case ITryMeElse itme -> {
					instructions[bytes++] = (byte) ITRY_ME_ELSE;
					instructions[bytes++] = (byte) (ipos[itme.retryPosition] >> 8 & 255);
					instructions[bytes++] = (byte) (ipos[itme.retryPosition] & 255);
				}
				case IUnify iunify -> {
	...
	public RC execute(Interpreter interpreter, boolean backtrackMode, gnu.prolog.term.Term args[]) throws PrologException
	{
		...
		{
			LeaveByteCodeBacktrackInfo lbi = switch (bi) {
				case LeaveByteCodeBacktrackInfo l -> l;
				default -> {
		...
		interpreter_loop: while (true) // interpreter loop
		{
			try
			{
				...
				catch (RuntimeException ex)
				{
					...
					logger.error("Runtime exception during bytecode execution", ex);
