...
public class TextInputPrologStream extends PrologStream
{
	...
	@Override
	public Term readTerm(Term streamTerm, Interpreter interpreter, ReadOptions options) throws PrologException
	{
		checkExists();
	...
	@Override
	public void writeTerm(Term streamTerm, Interpreter interpreter, WriteOptions options, Term term)
	...
	{
		checkExists();
		// For text streams, we need to peek ahead to detect EOF
		try
		{
			if (endOfStream != pastAtom)
			{
				// Try to peek ahead using mark/read/reset to see if we're at EOF
				termReader.mark(1);
				int rc = termReader.read();
				termReader.reset();
				if (rc == -1)
		...
		{
			debug(ex);
			// If we can't peek, return current state
		}
