...
public class TermWriter extends PrintWriter
{
	...
	/**
	 * Thread-local cached StringWriter and TermWriter for toString() operations.
	 * This avoids creating new objects on every toString() call.
	 */
	private static final ThreadLocal<StringWriter> cachedWriter = ThreadLocal.withInitial(StringWriter::new);
	private static final ThreadLocal<TermWriter> cachedTermWriter = new ThreadLocal<>();
	...
	/**
	 * Get or create a cached TermWriter for the current thread.
	 */
	private static TermWriter getCachedTermWriter() {
		final var writer = cachedWriter.get();
		var termWriter = cachedTermWriter.get();
		if (termWriter == null) {
			termWriter = new TermWriter(writer);
			cachedTermWriter.set(termWriter);
		}
		return termWriter;
	}
	 ...
	 */
	public static String toString(final Term term)
	{
		try
		{
			final var writer = cachedWriter.get();
			final var buffer = writer.getBuffer();
			buffer.setLength(0);  // Clear the buffer for reuse
			final var termWriter = getCachedTermWriter();
	 ...
	 * @return String representation of the term
	 */
	public static String toString(final Term term, final WriteOptions options)
	{
		try
		{
			final var writer = cachedWriter.get();
	...
	protected void displayTerm(WriteOptions options, int priority, Term term)
	{
		...
		if (term instanceof AtomTerm at)
		{
			boolean isOp = isOperator(options.operatorSet, at) && options.quoted;
			if (isOp)
			{
				print("(");
			}
			displayAtom(options, at);
			if (isOp)
			{
				print(")");
			}
		}
		else if (term instanceof IntegerTerm it)
		{
			displayInteger(options, it);
		}
		else if (term instanceof FloatTerm ft)
		{
			displayFloat(options, ft);
		}
		else if (term instanceof DecimalTerm dt)
		{
			displayDecimal(options, dt);
		}
		else if (term instanceof CompoundTerm ct)
		{
			displayCompound(options, priority, ct);
		}
		else if (term instanceof VariableTerm vt)
		{
			displayVariable(options, vt);
		}
		else if (term instanceof JavaObjectTerm jot)
		{
			displayJavaObject(options, jot);
		}
	}
	...
	protected void displayCompound(WriteOptions options, int priority, CompoundTerm term)
	{
		...
		if (options.numbervars && term.tag == numbervarsTag && term.args[0] instanceof IntegerTerm it)
		{
			int n = it.value;
			print((char) (n % 26 + 'A'));
			print(n / 26);
	...
	protected void displayList(WriteOptions options, CompoundTerm term)
	{
		...
		else if (tail instanceof CompoundTerm ct && ct.tag == TermConstants.listTag)
		{
			print(", ");
			displayList(options, ct);
		}
		else
		{
			print("|");
	...
	/**
	 * display decimal term
	 *
	 * @param options
	 *          current write options
	 * @param term
	 *          decimal term to write
	 */
	protected void displayDecimal(WriteOptions options, DecimalTerm term)
	{
		print(term.value.toString());
	}
	 ...
	 *          a quote of string
	 */
	protected static void appendQuotedChar(StringBuilder buf, char ch, char quote)
	{
