...
public class RandomAccessFileWriter extends Writer
{
	...
	{
		// Convert chars to bytes using UTF-8 and write in batch
		// This is more efficient than writing one char at a time with raf.writeChar()
		final var bytes = new String(buffer, offset, length).getBytes(java.nio.charset.StandardCharsets.UTF_8);
		raf.write(bytes);
	}
