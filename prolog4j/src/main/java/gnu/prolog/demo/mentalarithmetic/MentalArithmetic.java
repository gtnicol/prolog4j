...
{
	private static final Logger logger = LoggerFactory.getLogger(MentalArithmetic.class);
	...
	public static Pair<String, Integer> generateQuestion(int limit, int length) throws PrologException, NoAnswerException
	{
		...
		synchronized (interpreter)// so that this class is thread safe.
		{
			...
			if (rc == PrologCode.RC.SUCCESS || rc == PrologCode.RC.SUCCESS_LAST)
			{
				...
				// Compute left part (String from list)
				String leftPart;
				if (list != null)
				{
					if (list instanceof CompoundTerm)
					{
						...
						{// Turn it into a string to use.
							leftPart = TermWriter.toString(list);
						}
						else
						{
				...
				// Compute right part (Integer from value)
				int rightPart;
				if (value != null)
				{
					if (value instanceof IntegerTerm)
					{
						rightPart = ((IntegerTerm) value).value;
					}
					else
					{
				...
				// Create the immutable answer pair
	...
	{
		final var errors = env.getLoadingErrors();
		for (final var error : errors)
		{
			logger.warn("Prolog loading error: {}", error);
		}
	}
}
